...
//  CUSTOMER_ID = await createCustomer().then(({ id }) => id)
  ...
  await open(`http://localhost:${SERVER_PORT}/yunique`);
...
async function createCustomer(customerData) {
  try {
      const response = await fetch('https://api-sandbox.y.uno:443/v1/customers', {
          method: 'POST',
          headers: {
              'public-api-key': PUBLIC_API_KEY,
              'private-secret-key': PRIVATE_SECRET_KEY,
      ...
      return responseData; // Devuelve la respuesta de la API de Yuno
  } catch (error) {
...
app.post('/crear-cliente', async (req, res) => {
  const { merchant_customer_id, first_name, last_name, email, orderId2} = req.body;
  ...
  try {
      ...
      merchant_cust_id = responseData.merchant_customer_id;
...
app.get('/crear-cliente', (req, res) => {
  res.sendFile(path.join(__dirname, 'crear-cliente.html'));
  
});

app.get('/yunique', (req, res) => {
  res.sendFile(path.join(__dirname, 'yunique.html'));
});